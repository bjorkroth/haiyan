...
{
    public class ColumnModelElementMapper : IModelElementMapper
    {
        private readonly IfcStore _model;
        private readonly IMaterialBuilder _materialBuilder;

        public ColumnModelElementMapper(IfcStore model, IMaterialBuilder materialBuilder)
        {
            _model = model;
        ...
        public bool CanApply(IIfcProduct product)
        {
            ...
            return product.GetType().Name == nameof(IfcColumn);
        }

        public HaiyanBuildingElement MapBuildingElement(IIfcProduct product)
        {
            return new HaiyanColumn
            {
                Name = product.Name ?? "",
